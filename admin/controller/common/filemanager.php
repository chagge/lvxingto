...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		// Check user has permission验证用户权限
		if (!$this->user->hasPermission('modify', 'common/filemanager')) {
			$json['error'] = $this->language->get('error_permission');
		}

		// Make sure we have the correct directory验证目录是否正确
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'catalog/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'catalog';
		...
		if (!$json) {
			if (!empty($this->request->files['file']['name']) && is_file($this->request->files['file']['tmp_name'])) {
				...
				$filename = ip2long($this->request->server['REMOTE_ADDR']).date("Ymdhis").rand(10000,99999).strrchr(basename(html_entity_decode($this->request->files['file']['name'], ENT_QUOTES, 'UTF-8')), '.');
