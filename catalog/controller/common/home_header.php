...
class ControllerCommonHomeheader extends Controller {
	public function index() {
		...
		$data['text_full_screen_nv'] = $this->language->get('text_full_screen_nv');
		...
		$data['businesses'] = $this->url->link('seller/admin');
		...
		if ($this->config->get('config_full_screen')==TRUE) {//是否通栏显示
			$data['home_full_screen'] = '#02C0AC';
		} else {
			$data['home_full_screen'] = '';
		}
		
		if ($this->config->get('config_home_nvgap')) {//首页导航下的间隙高度
			$data['home_nvgap'] = $this->config->get('config_home_nvgap');
		} else {
			$data['home_nvgap'] = '';
		}
		
		$token='qqqqqqqqqqqqqqqqqqq'; $encodingAesKey='yyyyyyyyyyyyyyy'; $appId='cccccccc';
		$c=$this->wxbizmsgcrypt->wxbizmsgcrypt($token, $encodingAesKey, $appId);
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/home_header.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/common/home_header.tpl', $data);
		} else {
			return $this->load->view('default/template/common/home_header.tpl', $data);
		}
	}
}